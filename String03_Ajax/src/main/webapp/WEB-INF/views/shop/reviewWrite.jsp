...
<style>
	...
	#reviewList li:ntn-child(3n+1){ /* 이미지 */
		width:30%;
	}
	#reviewList li:ntn-child(3n+2){ /* 상품평 */
		width:70%;
	}
	#reviewList li:ntn-child(3n){
			...
			<div class="modal-dialog modal-xl">
				<div class="modal-content">
					...
					<div class="modal-body">
						<div class="row">
							<!--리뷰 쓰기 xl,lg,md(desktop), sm(tablet) xs(mobile)  -->
							<div class="col-md-6 col-sm-12 col-xs-12">
								...
								<form name="rf" id="rf" method="post"
									...
									<input type="hidden" name="pnum" id="pnum" value="${param.pnum }"> 
									<input type="text" name="no" id="no" value="0">
									<!-- 글수정시 사용 -->
...
<script>
	...
	function send(){
		let isChk=$('input[name="score"]').is(":checked"); //체크하면 true
		//alert(isChk)
		if(!isChk){
			alert('평가점수를 체크하세요');
			return;
		...
		//alert(btnTxt);
		// [1] 파일을 업로드 하지 않을 경우. input값들을 파라미터 문자열로 만들어서 전송해야 함
		let paramData = $('#rf').serialize(); // 직렬화
		//alert(paramData);
		
		// [2] 파일을 업로드 할 경우. FormData객체를 이용
		let form = $('#rf')[0] //form객체
		//alert(form)
		let formData = new FormData(form); //파일 업로드시는 FormData를 전송한다
		//formData.append("name", "Sam"); // key,value

		if(btnTxt=='글쓰기'){
			//writeReview(paramData) //파일 업로드 하지 않을 경우
	...
	function writeReviewAndFile(formData){
		$.ajax({
			...
			processData:false,// 일반적으로 서버에 전달하는 데이터는 query string형태. false를 주면 query string을 사용 안함
	...
	//PUT	/reviews ==>수정처리(파일업로드 안할 때)
	//Post	/reviews ==> 수정처리(파일업로드할 때는 POST로 하자)
	function updateReviewAndFile(formData){
/* 		let file = $('#filename')[0].value;
		formData.append("filename", file);
		alert(file); */
		formData.append("mode", "edit"); //mode값이 edit이면 수정처리 ==> 컨트롤러에서, 비어있으면 등록 처리
	...
	function showList(res){ //res => 배열이므로 반복문 사용해야함
		let str = "";
		$(res).each(function(i, obj){ // obj에 리뷰 글이 담김
			...
			if(obj.filename == null){
				str += `<li><img src="resources/images/noimage2.PNG" class="img img-thumbnail"></li>`;
			}else{
				str += `<li><img src="resources/images/\${obj.filename}" class="img img-thumbnail" style="width:30%"></li>`;
			}//else
	...
	//DELETE reviews/22		==> 경로 접근 방식으로 글번호(no)를 넘긴다.
	function remove(no){
		$.ajax({
			type:'delete',
			url:'reviews/' + no,
	...
	//GET /reviews/22	==> 특정 게시글 가져오기(조회->업데이트)
	function edit(no){
		$.ajax({
			type:'get',
			url:'reviews/' + no,
		...
		.done((res)=>{
			//alert(JSON.stringify(res))
			$('#score'+res.score).prop('checked', true); //스코어 점수 체크
