...
<script>
 	...
 	function chat_connect(){
 		...
 		stompClient = Stomp.over(socket); //소켓을 이용해서 stomp생성
 		//stomp이용해서 서버에 연결
 		stompClient.connect({}, function(frame){ //{ } : 연결옵션 없음 / frame : 연결이 성공했을 때 실행되는 콜백 함수
 			//alert('연결됨 : ' + frame);
 			$('#status').html(nickname + "님이 챗서버에 연결되었습니다.");
 			setConnected(true); //setConnected() 함수를 호출
			...
			sendMessage(nickname,"all", nickname+"님이 접속했습니다");
 			...
 			stompClient.subscribe("/topic/messages", function(msg){
 				//alert(msg.body) //msg.body==>json형태의 문자열(string)
 				
 				let jsonMsg = JSON.parse(msg.body);//msg.body를 json객체로 바꾸자
 				//alert(jsonMsg.text)
 	...
 	function sendInput(mymsg){
 		//alert(event.keyCode); // 입력한 아스키코드값 => enter : 13
	...
	<div class="container">
		<div class="row">
			<div class="col-md-12">
				...
				<!-- 접속하기 -->
				<button type="button" class="btn btn-warning" id="connectBtn"
					onclick="chat_connect()">채팅서버 연결하기</button>
				<!-- 접속끊기 -->
				<button type="button" class="btn btn-secondary" id="disConnectBtn"
					onclick="chat_disconnect()">채팅서버 연결끊기</button>
			</div>
		</div>
		...
		<div id="showChat" style="display: none">
			...
			<div class="mb-3 mt-3">
				<label for="inputMsg" class="form-label">메시지 : </label> <input
					onkeyup="sendInput(this.value)" type="text" class="form-control"
					id="inputMsg" placeholder="메시지를 입력하세요." name="inputMsg">
			</div>
			<!-- 대화 내용 -->
			<div id="taMsg"></div>
		</div>
