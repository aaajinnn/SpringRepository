...
public class LoginCheckInterceptor implements HandlerInterceptor{
	...
	//[1]preHandle() : 사전처리 Controller를 실행하기 전에 호출되는 메서드
	//[2]postHandle() : 사후처리 Controller를 실행한 후, 아직 뷰를 실행하기 전에 호출되는 메서드
	//[3] afterCompletion() : 사후처리 Controller를 실행하고 뷰를 실행한 후에 호출되는 메서드
	
	@Override
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
			throws Exception {
		
		HttpSession ses = request.getSession(); //세션얻어옴
		MemberVO user = (MemberVO)ses.getAttribute("loginUser"); // 로그인 시 키값 얻어옴
		if(user==null) {
			request.setAttribute("msg", "로그인해야 이용하실 수 있습니다.");
		...
		return true; //true를 반환하면 컨트롤러가 실행됨
	}
