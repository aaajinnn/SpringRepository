...
public class BoardController {
	...
	public String boardList(Model m, PagingVO page, HttpServletRequest req) {
		...
		page.setTotalCount(totalCount); // 전체 게시글 수
		...
		// 2. 게시글 목록 가져오기
		List<BoardVO> boardList = bService.getBoardAll(page);
		
		// 3. 페이지 네비게이션 문자열 받아오기
		String mystx = req.getContextPath();
		String loc = "board/list";
		String pageNavi = page.getPageNavi(mystx, loc);
		...
		//페이징처리
		m.addAttribute("page", page); //VO에 페이징할 변수들이 저장되어 있기때문에 이렇게 불러옴
	...
	public String boardReWriteForm(Model m, BoardVO vo) { //vo에서 글번호와 제목 가져오기
		log.info("vo : " + vo); //부모글 글번호(num), 글제목(subject)
