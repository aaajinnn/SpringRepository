...
public class ShopServiceImpl implements ShopService {
	...
	public int addCart(CartVO cartVo) {
		...
		//[1] userid와 pnum으로 cart테이블에서 장바구니 번호 가져오자
		Integer cnum = cartMapper.selectCartNum(cartVo);
		int n=0;
		if(cnum==null) {//[1_1] 새로 담는 상품인 경우
			n = cartMapper.addCart(cartVo);
		}else { //[1_2] 이미 담긴 상품인 경우
	...
	public int editCart(CartVO cartVo) {
		...
		if(qty > 50) {
			throw new NumberFormatException("수량은 50개 이내로만 수정 가능합니다.");//Runtime계열 예외는 throws 생략 가능
		}else if(qty > 0)
			return cartMapper.editCart(cartVo);//수정처리
		
		else if(qty==0) {
			return cartMapper.delCart(cartVo.getCnum());//삭제처리
		}else { //음수인 경우
			throw new NumberFormatException("수량은 음수로 입력할 수 없습니다.");
		}
	}
