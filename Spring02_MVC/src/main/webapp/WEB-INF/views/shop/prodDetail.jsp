...
<script>
	const openPop = function(img){
		...
		let h = obj.height;
		open(url, "preview", "width=" + w + "px, height=" + h + "px, letf=100px, top=100px");
		
	}//--------------------
		const goCart = function(){
			frm.action = 'user/cartAdd'; //form name으로 접근할 때
			//frm.method='get'; //나중에 post로 수정하자
			frm.submit();
		}
		
		const goOrder = function(){ //jquery로 접근할 때
			//jquery에서 속성 설정 메서드 : attr(속성명, 값)=>정적인속성, prop(속성명, 값)=>기능적인속성
				...
				<tr>
					...
					<td align="left" width="50%" style="padding-left: 40px">
						<h4>
							...
							할인율:<span style="color: red">${item.percent } %</span><br> POINT:<b
						style="color: green">[${item.point }]</b>POINT<br>
