...
table{
	...
	table-layout: fixed;/* 테이블 너비를 고정시킴 */
	overflow:auto;
}
td:nth-child(2n) {
	text-align: left;
...
<div class="container">
	...
	<c:choose>
		<c:when test="${vo eq null }">
			<h3 class="text-center">해당 게시글은 존재하지 않아요</h3>
		</c:when>
		<c:otherwise>
			...
			<table class="table table-bordered">
				...
				<tr>
					...
					<c:if test="${vo.filename ne null }">
						<a href="${pageContext.request.contextPath }/resources/board_upload/${vo.filename}" download>
						 ...
						 <!-- 첨부파일명을 모두 소문자로 변환해서 fname 변수에 할당 ==> 확장자 체크 위해-->
				...
				<tr>
					...
					<a href="javascript:goRe()">답변쓰기</a></td>
				</tr>
			</table>
		</c:otherwise>
	</c:choose>
	...
	<form id="bf" name="bf" method="post">
		...
		<div id="divPwd" style="display: none"
			...
			<label for="pwd">비밀번호</label> <input type="password" name="passwd"
	...
	<form name="reF" id="reF" action="${myctx}/user/rewrite" method="post">
		...
		<input type="hidden" name="num" value="<c:out value="${vo.num }"/>"> <input
			type="hidden" name="subject" value="<c:out value="${vo.subject }"/>">
	</form>
	<!-- ---------------------------------------------- -->
