...
<%@ include file="/WEB-INF/views/inc/top.jspf" %>
<h2 class="text-center">상품등록 - Product Form</h2>
<div class="row">
		...
		<form name="pf" id="pf" action="prodInsert" method="post" enctype="multipart/form-data">
			<table class="table">
				<tr>
					<th width="20%">카테고리</th>
					<td width="80%">
						<!-- 상위 카테고리 --> <select name="upCg_code" id="upCg_code">
							<option value="">::상위 카테고리::</option>
							<c:forEach var="up" items="${upCgList }">
								<option value="${up.upCg_code}">${up.upCg_name }</option>
							</c:forEach>
					</select> <!-- 하위 카테고리 --> <span id=downCg> <select
							name="downCg_code" id="downCg_code">
...
<%@ include file="/WEB-INF/views/inc/foot.jspf" %>